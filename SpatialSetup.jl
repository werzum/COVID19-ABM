...
function fill_map(model,group,long, lat, correction_factor,schools,schoolrange, workplacedict, social_groups,distant_groups)
    ...
    for x in 1:inhabitants
        agent_properties[Int(x)] = agent_tuple(:S,0,0,0,0,false,age,0,0,0,0,0,undef_vector,undef_vector,undef_vector)
    end
    ...
    for agent in agent_properties
        ...
        add_agent!(agent.household, model, agent.health_status, agent.days_infected, agent.attitude, agent.fear, agent.behavior, agent.women, agent.age, agent.wealth, agent.household, agent.workplace, agent.socialgroup, agent.distantgroup, agent_workplace_route, agent_social_route, agent_distant_route)
    end
    return
end
